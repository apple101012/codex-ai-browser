...
describeBrowser("PlaywrightRuntime smoke", () => {
  ...
  it("marks profile stopped when last tab is closed", async () => {
    const root = await createTempDir("runtime-close-tab-");
    dirsToClean.push(root);
    ...
    try {
      ...
      const closeResult = await runtime.execute(profile, {
        type: "closeTab"
      });
      expect(closeResult.ok).toBe(true);
      expect((closeResult.data as { tabs: unknown[] }).tabs).toHaveLength(0);
      expect(runtime.isRunning(profile.id)).toBe(false);
    } finally {
      await runtime.stopAll();
